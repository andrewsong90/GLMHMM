...
def _fit_transition_filters(stim, symb, gamma, xi, trans_w, options, train_data):
    ...
    if options['evaluate'] == True:
        ...
        for i in range(0, options['num_states']):
            outweights = _minimize_LBFGS(lambda x: _trans_learning_fun(x, new_stim, i, options), np.reshape(trans_w[i, :, :].T, (trans_w.shape[1] * trans_w.shape[2]), order = 'F'), lr = 1, max_iter = 500, tol = 1e-5, line_search = 'Wolfe', interpolate = True, max_ls = 25, history_size = 100, out = False)
