...
def _GLMHMM_symb_lik(emit_w, X_trial, y_trial):
    ...
    # print(X_trial.shape, emit_w.shape)
    ...
    for t in range(0, len(y_trial)):
        ...
        if np.any(np.isnan(symb_lik[:, t])):
            print('Symb Oh dear! ', t)
            # print(1 / (1 + np.sum(np.exp(np.sum(emit_w * X_trial_mod[:, :, :, t], axis = 2)), axis = 1)))
            # print(symb_lik[:,t])
            # print("+++++++++++")
            # print(np.sum(emit_w[:, int(y_trial[t]) - 1, :] * X_trial_mod[:, int(y_trial[t]) - 1, :, t], axis = 1))
            # print("+++++++++++")
            # print(emit_w[:, int(y_trial[t]) - 1, :])
            # print("==========")
            # print(X_trial_mod[:, int(y_trial[t]) - 1, :, t])
    return symb_lik
