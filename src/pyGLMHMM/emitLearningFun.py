...
def _emit_learning_fun(emit_w, stim, state_num, options):
    ...
    for trial in range(0, len(stim)):
        ...
        # mat1 = np.tile(np.expand_dims(emit_w, axis = 2), (1, 1, T))
        mat1 = np.zeros((num_states, num_total_bins, T))
        for idx in range(T):
            mat1[..., idx] = emit_w

        # mat2 = np.tile(np.reshape(stim[trial]['data'], (1, num_total_bins, T), order = 'F'), (num_states, 1, 1))
        mat2 = np.zeros((num_states, num_total_bins, T))
        for idx in range(num_states):
            mat2[idx] = np.reshape(stim[trial]['data'], (1, num_total_bins, T), order = 'F')

        filtpower = np.reshape(
                                    np.sum(
                                            np.reshape(
                                                        mat1, (num_states, num_total_bins, T), order = 'F'
                                                    ) * mat2, axis = 1
                                        ),
        ...
        denom = np.zeros((num_states, T))
        temp = 1 + np.expand_dims(np.sum(np.exp(filtpower), axis = 0), axis=0)
        for idx in range(num_states):
            denom[idx] = temp
