    ...
    if options['evaluate'] == True:
        ...
        for i in range(0, options['num_states']):
            if options['num_steps'] == 1:
                outweights = _minimize_LBFGS(lambda x: _emit_learning_fun(x, new_stim, i, options), np.reshape(emit_w[i, :, :].T, (emit_w.shape[1] * emit_w.shape[2]), order = 'F'), lr = 1, max_iter = 500, tol = 1e-5, line_search = 'Wolfe', interpolate = True, max_ls = 25, history_size = 100, out = False)
            else:
                outweights = _minimize_LBFGS(lambda x: _emit_multistep_learning_fun(x, new_stim, i, options), np.reshape(emit_w[i, :, :].T, (emit_w.shape[1] * emit_w.shape[2]), order = 'F'), lr = 1, max_iter = 500, tol = 1e-5, line_search = 'Wolfe', interpolate = True, max_ls = 25, history_size = 100, out = False)
